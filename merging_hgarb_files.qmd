...
setdiff(filelist_2022, filelist_2022_old) -> new_list
...
read_1_file("") %>% select(full_name, plan_id)
```
...
df_newlist %>% 
filter(full_name == "Alaska-Teacher's Retirement System")
...
all_plans_2021_2022 %>% add_count(full_name) %>% select(full_name, n) %>% 
  filter(n < 21 | n > 22)
all_plans_2021_2022 %>% filter(full_name == "Wyoming Retirement System, Public Employees’ Pension Plan")
...
all_plans_2021_2022_all_ids %>% 
  filter(str_detect(plan_name_hgarb, "(?i)Massachusetts")) %>% write.csv("all_plans_massachusetts.csv")
```

# reason complied names & ppd
```{r}
#write.csv(all_plans_2021_2022_all_ids, "output/all_plans_2021_2022.csv")
write.csv(all_plans_2021_2022_all_ids, "output/all_plans_2021_2022_updated April 18.csv")
...
<!-- # Equable teacher plans -->
<!-- ```{r} -->
<!-- teachers_plans_equable <- import("data/List of Teacher and Public School Plans for Reason.xlsx") %>% filter(`Municipal Plan` != "Yes") %>% filter(`Who it Covers?` == "Teachers") %>% select(System_Name, Plan_Shorthand) %>%  -->
...
<!-- teachers_plans_equable %>% left_join(ppd_plans_name, by = c("Plan_Shorthand" = "PlanName")) -->

<!-- ppd_plans_name %>% filter(PlanName %in% teachers_plans_equable$Plan_Shorthand) -->

<!-- teachers_plans_equable$Plan_Shorthand_equable -> equable_teacher_plan_list -->
<!-- teachers_plans_equable$System_Name -->

<!-- reason_plans_covering_teachers <- all_plans_2021_2022_all_ids %>% left_join(ppd_plans_name, by = "ppd_id") %>%  -->
<!--   filter(plan_name_hgarb != "MICHIGAN PUBLIC SCHOOL EMPLOYEES RETIREMENT SYSTEM") %>%  -->
<!--    # Regex to filter all plans that cover teacher - apply for reason data -->
<!--   filter(str_detect(plan_name_hgarb, "(?i)(teacher)|(teachers)|(school)|(TRS)|(Education)") | str_detect(short_hand_ppd, "(?i)teacher|(Virginia RS)"))  -->

<!-- write.csv(reason_plans_covering_teachers, "plans_covering_teachers.csv") -->


<!-- reason_plans_covering_teachers %>% select(plan_name_hgarb, short_hand_ppd) %>% distinct() -->
<!-- # double check against equable - has 45 distince plans while equable has 40 -->
<!--   select(plan_name_hgarb, short_hand_ppd)  %>% distinct() -->



<!-- teachers_plans_equable %>% select(System_Name, Plan_Shorthand_equable) %>% distinct() -->

<!--   select(ppd_id, plan_id, plan_name_hgarb, short_hand_ppd) %>% distinct()  -->

<!-- teachers_plans_equable %>% filter(str_detect(System_Name, "(?i)Colorado")) -->
...
```{r}
all_plans_2021_2022_all_ids %>% filter(str_detect(plan_name_hgarb, "Connecticut")) %>% select(plan_name_hgarb) %>% distinct()
