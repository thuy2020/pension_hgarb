...
6, 8, 35, 76, 78, 131, 132 are dealt with below \### Arizona
...
all_plans_21_22_longform_database %>% left_join(reason_colnames, by= c("field" = ""))
...
all_plans_2021_2022  %>% filter(!is.na(vd))
  select(full_name, fye, payroll_from_acfr) %>% 
filter(!payroll_from_acfr %in% c("NA", "N/A")) %>% 
  mutate(payroll_from_acfr = as.double(payroll_from_acfr)) %>% 
  arrange(desc(payroll_from_acfr))
...
# 2021 has 48 plans that 2022 doesnt have (1 plan without "updated" in their name)
